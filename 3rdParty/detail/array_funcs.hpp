...
namespace Bitset2
{
    namespace detail
    {
        ...
        struct array_funcs
        {
            ...
            enum : size_t
            {
                base_t_n_bits = h_types<T>::base_t_n_bits,
                npos = h_types<T>::npos
            };
            ...
            constexpr bool none(array_t const& arr) const noexcept
            {
            ...
            {
                return sum_impl(count_impl(arr, std::make_index_sequence<n_array>()));
            }

            constexpr bool equal(array_t const& arr1, array_t const& arr2) const noexcept
            {
            ...
            /// of elements in arr1 and arr2
            template <class F>
            constexpr bool zip_fold_or(array_t const& arr1, array_t const& arr2, F& f) const
                noexcept(noexcept(f(base_t(0), base_t(0))))
            {
            ...
            template <size_t n>
            constexpr array_t copy_and_map(base_t const pttrn,
                                           typename h_types<T>::template array_t<n> const& arr) const noexcept
            {
                return n_array == 0 ?
                           array_t{} :
                           copy_and_map_impl(pttrn, arr, gen_empty_array<n_array, T>(), n >= n_array,
            ...
            constexpr array_t copy_and_map_impl(base_t const pttrn, typename h_types<T>::template array_t<n> const& arr,
                                                array_t const& zeroes, bool const take_all, std::index_sequence<S1...>,
                                                std::index_sequence<S2...>) const noexcept
            {
                return {{arr[S1]..., zeroes[S2]..., base_t((take_all ? arr[n_array - 1] : base_t(0)) & pttrn)}};
            }
            ...
            template <size_t... S>
            constexpr array_p1_t append_impl(array_t const& arr, base_t const v1, std::index_sequence<S...>) const
                noexcept
            {
                return {{arr[S]..., v1}};
            }
            ...
            template <size_t... S>
            constexpr array_p1_t prepend_impl(base_t const v1, array_t const& arr, std::index_sequence<S...>) const
                noexcept
            {
                return {{v1, arr[S]...}};
            }
            ...
            {
                return (arr1[ct] < arr2[ct])
                       || (arr1[ct] == arr2[ct] && (ct == 0 ? false : less_than_impl(arr1, arr2, ct - 1)));
            }
            ...
            template <size_t... S>
            constexpr array_t bitwise_op_impl(op_type opt, array_t const& arr1, array_t const& arr2,
                                              std::index_sequence<S...>) const noexcept
            {
                return {{h_bitwise_op(S, opt, arr1, arr2)...}};
            }

            constexpr base_t h_bitwise_op(size_t idx, op_type opt, array_t const& arr1, array_t const& arr2) const
                noexcept
            {
                return (opt == op_type::or_op) ?
                           (arr1[idx] | arr2[idx]) :
                           ((opt == op_type::and_op) ?
                                (arr1[idx] & arr2[idx]) :
                                (opt == op_type::xor_op) ? (arr1[idx] ^ arr2[idx]) : (arr1[idx] & ~arr2[idx]));
            }

            /// Count bits in each element of arr
            template <size_t... S>
            constexpr std::array<size_t, n_array> count_impl(array_t const& arr, std::index_sequence<S...>) const
                noexcept
            {
            ...
            template <class T1>
            constexpr T1 sum_impl(std::array<T1, n_array> const& vals, size_t ct = n_array - 1) const noexcept
            {
                return vals[ct] + ((ct == 0) ? T1(0) : sum_impl(vals, ct - 1));
            }

            constexpr size_t idx_lsb_set(array_t const& arr, base_t v, size_t idx) const noexcept
            {
                return v == base_t(0) ? (idx + 1 == n_array ? npos : idx_lsb_set(arr, arr[idx + 1], idx + 1)) :
