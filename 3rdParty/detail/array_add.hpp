...
struct array_add_base
{
    ...
    template <size_t n>
    using array_pair_t = std::pair<base_t, typename h_types<T>::template array_t<n>>;

    constexpr array_pair_t<it_n + 1> add_h1(base_t a, base_t b, base_t sm1, base_t sum,
                                            typename h_types<T>::template array_t<it_n> const& so_far) const noexcept
    {
        return std::make_pair(
            (sum < a || sm1 < b) ? base_t(1) : base_t(0),
            array_funcs<it_n, T>().append(so_far, (it_n + 1 < n_array) ? sum : base_t(sum & hgh_bit_pattern)));
    }

    constexpr array_pair_t<it_n + 1> add_h2(base_t a, base_t b, array_pair_t<it_n> const& a_p) const noexcept
    {
...
struct array_add_h : public array_add_base<N, it_n, T>
{
    ...
    template <size_t n>
    using array_pair_t = typename array_add_base<N, it_n, T>::template array_pair_t<n>;

    constexpr array_pair_t<it_n + 1> add_h3(array_t const& arr1, array_t const& arr2) const noexcept
    {
        return this->add_h2(arr1[it_n], arr2[it_n], array_add_h<N, it_n - 1, T>().add_h3(arr1, arr2));
    }
}; // struct array_add_h
...
struct array_add_h<N, 0, T> : public array_add_base<N, 0, T>
{
    ...
    constexpr array_pair_t<1> add_h3(array_t const& arr1, array_t const& arr2) const noexcept
    {
        return this->add_h2(arr1[0], arr2[0], std::make_pair(T(0), zero_array_t{}));
    }
}; // struct array_add_h
...
struct array_add
{
    ...
    constexpr array_t add(array_t const& arr1, array_t const& arr2) const noexcept
    {
        return (N == 0) ? array_t{} : array_add_h<N, n_array - 1, T>().add_h3(arr1, arr2).second;
    }

    constexpr void add_assgn(array_t& arr1, array_t const& arr2) const noexcept
    {
