...
struct array_complement2_base
{
    ...
    constexpr arr_t<it_n + 1> comp2_h1(arr_t<it_n> const& so_far, base_t sum) const noexcept
    {
        return array_funcs<it_n, T>().append(so_far, (it_n + 1 < n_array) ? sum : base_t(sum & hgh_bit_pattern));
    }

    constexpr arr_t<it_n + 1> comp2_h2(array_t const& arr, arr_t<it_n> const& so_far, base_t carry) const noexcept
    {
        return comp2_h1(so_far, base_t(base_t(~arr[it_n]) + carry));
    }

    constexpr arr_t<it_n + 1> comp2_h3(array_t const& arr, arr_t<it_n> const& so_far) const noexcept
    {
        return comp2_h2(arr, so_far, it_n == 0 ? base_t(1) : base_t(so_far[it_n - 1] == 0 && arr[it_n - 1] == 0));
    }
...
struct array_complement2_h : array_complement2_base<N, it_n, T>
{
    ...
    constexpr arr_t<it_n + 1> comp2_h4(array_t const& arr) const noexcept
    {
        return this->comp2_h3(arr, array_complement2_h<N, it_n - 1, T>().comp2_h4(arr));
    }
...
struct array_complement2_h<N, 0, T> : array_complement2_base<N, 0, T>
{
    ...
    constexpr arr_t<1> comp2_h4(array_t const& arr) const noexcept
    {
...
struct array_complement2
{
    ...
    constexpr array_t comp2(array_t const& arr) const noexcept
    {
        return (N == 0) ? array_t{} : array_complement2_h<N, n_array - 1, T>().comp2_h4(arr);
    }
