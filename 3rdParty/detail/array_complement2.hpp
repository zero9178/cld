...
namespace Bitset2
{
    namespace detail
    {
        ...
        struct array_complement2_base
        {
            ...
            {
                return array_funcs<it_n, T>().append(so_far,
                                                     (it_n + 1 < n_array) ? sum : base_t(sum & hgh_bit_pattern));
            }

            constexpr arr_t<it_n + 1> comp2_h2(array_t const& arr, arr_t<it_n> const& so_far, base_t carry) const
                noexcept
            {
            ...
            constexpr arr_t<it_n + 1> comp2_h3(array_t const& arr, arr_t<it_n> const& so_far) const noexcept
            {
                return comp2_h2(arr, so_far,
                                it_n == 0 ? base_t(1) : base_t(so_far[it_n - 1] == 0 && arr[it_n - 1] == 0));
            }
