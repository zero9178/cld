...
protected:
    ...
    /* ----------------------------------------------------------------------- */
    constexpr bitset2_impl() noexcept {}

    constexpr bitset2_impl(bitset2_impl const&) noexcept = default;

    constexpr bitset2_impl(bitset2_impl&&) noexcept = default;

    constexpr bitset2_impl& operator=(bitset2_impl const&) noexcept = default;

    constexpr bitset2_impl& operator=(bitset2_impl&&) noexcept = default;

    explicit constexpr bitset2_impl(ULLONG_t v) noexcept : m_value(ullong2array<N, T>()(v)) {}

    template <size_t n, class Tsrc>
    explicit constexpr bitset2_impl(std::array<Tsrc, n> const& value) noexcept
        : m_value(a2a<n, Tsrc>()(hgh_bit_pattern, value))
    {
    }

    explicit bitset2_impl(const std::bitset<N>& bs) noexcept
    {
        if (N == 0)
            return;
    ...
    explicit bitset2_impl(std::basic_string<CharT, Traits, Alloc> const& str,
    ...
    constexpr array_t& get_data() noexcept
    {
        return m_value;
    }
    ...
    constexpr bitset2_impl& set() noexcept
    {
        if (N > 0)
        {
            size_t c = 0;
    ...
    constexpr bitset2_impl& reset() noexcept
    {
    ...
    constexpr bitset2_impl& flip_noexcept(size_t bit) noexcept
    {
    ...
    constexpr bitset2_impl& flip() noexcept
    {
        if (N > 0)
        {
            size_t c = 0;
...
public:
    constexpr array_t const& data() const noexcept
    {
        return m_value;
    }
    ...
    constexpr size_t find_next(size_t idx) const
    {
        ...
        return idx + 1 == N ?
                   ...
                   detail::array_funcs<n_array, T>().idx_lsb_set(
                       m_value,
                       base_t(m_value[(idx + 1) / base_t_n_bits] & ce_left_shift(T(~T(0)), (idx + 1) % base_t_n_bits)),
                       (idx + 1) / base_t_n_bits);
    }

    constexpr bool operator==(bitset2_impl const& v2) const noexcept
    {
    ...
    explicit operator std::bitset<N>() const
    {
        ...
        if (N == 0)
            return b_t{};
        if (n_words == 1)
            return b_t(to_ullong());

        b_t ret_val;
        for (size_t ct = 0; ct < N; ++ct)
            ret_val[ct] = operator[](ct);
