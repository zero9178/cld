...
struct ullong2array
{
    ...
    constexpr array_t operator()(ULLONG_t v) const noexcept
    {
        return fill(gen_empty_array<n_array, T>(), v, std::make_index_sequence<n_empty_vals>(),
                    std::make_index_sequence<centrl_i>());
    }

    template <size_t... S1, size_t... S2>
    constexpr array_t fill(array_t const& empty, ULLONG_t v, std::index_sequence<S1...>,
                           std::index_sequence<S2...>) const noexcept
    {
        return {{base_t(ce_right_shift(v, S2 * base_t_n_bits))...,
                 base_t(ce_right_shift(v, centrl_i * base_t_n_bits) & use_pattern), empty[S1]...}};
    }
}; // struct ullong2array
