...
struct array2array
{
    ...
    enum : size_t
    {
        ts_div = small_to_large ? trgt_base_n_bits / src_base_n_bits : src_base_n_bits / trgt_base_n_bits
    };
    ...
    template <size_t... S1>
    constexpr trgt_array_t conv_small_to_large(Ttrgt pttrn, src_array_t const& src,
    ...
    template <size_t... S1>
    constexpr trgt_array_t conv_large_to_small(Ttrgt pttrn, src_array_t const& src,
                                               std::index_sequence<S1...>) const noexcept
    {
        return {{get_from_larger(S1 / ts_div, S1 % ts_div, src)...,
                 Ttrgt(get_from_larger((trgt_n - 1) / ts_div, (trgt_n - 1) % ts_div, src) & pttrn)}};
    }

    constexpr Ttrgt get_from_smaller(size_t trgt_idx, src_array_t const& src, size_t src_idx, size_t src_ct = 0,
