...
struct array_ops
{
    ...
    constexpr array_ops(size_t n_shift) noexcept
        : m_n_shift_mod(n_shift % N),
          ...
          m_shft_left_pattern(ce_left_shift(T(~T(0)), base_t_n_bits - (n_shift % base_t_n_bits))),
          m_shft_right_pattern(ce_right_shift(T(~T(0)), base_t_n_bits - (n_shift % base_t_n_bits)))
    {
    }

    constexpr array_t shift_left(array_t const& arr) const noexcept
    {
        return shift_left_impl(arr, std::make_index_sequence<n_array>());
    }

    constexpr array_t shift_right(array_t const& arr) const noexcept
    {
    ...
    constexpr void shift_left_assgn(array_t& arr) const noexcept
    {
        for (size_t c = n_words; c > 0;)
        {
            ...
            if (c >= m_shft_div)
            {
                ...
                base_t const v2 = c2 == 0 ?
                                      base_t(0) :
                                      ce_right_shift(base_t(arr[c2 - 1] & m_shft_left_pattern), m_shft_leftright_shift);
                arr[c] = base_t(v1 | v2);
            }
            else
                arr[c] = base_t(0);
    ...
    {
        return (n_array == 1) ?
                   array_t{{base_t((base_t(arr[0] << m_n_shift_mod) | (ce_right_shift(arr[0], N - m_n_shift_mod)))
                                   & hgh_bit_pattern)}} :
                   rotate_left_impl(arr, array_ops<N, T>(m_n_shift_mod), array_ops<N, T>(N - m_n_shift_mod),
                                    std::make_index_sequence<n_array>());
    } // rotate_left

    constexpr array_t flip(array_t const& arr) const noexcept
    {
        return flip_impl(arr, std::make_index_sequence<n_array>());
    }

    constexpr bool all(array_t const& arr) const noexcept
    {
    ...
    template <size_t... S>
    constexpr array_t shift_left_impl(array_t const& arr, std::index_sequence<S...>) const noexcept
    {
        return {{h_shift_left(S, arr)...}};
    }

    constexpr base_t h_shift_left(size_t idx, array_t const& arr) const noexcept
    {
        return h_shift_left1(idx, arr) & ((idx + 1 == n_words) ? hgh_bit_pattern : all_one);
    }

    constexpr base_t h_shift_left1(size_t idx, array_t const& arr) const noexcept
    {
        return (idx >= n_words || idx < m_shft_div) ?
                   base_t(0) :
                   base_t((arr[idx - m_shft_div] << m_shft_mod) | h_shift_left_rem(idx - m_shft_div, arr));
    }

    constexpr base_t h_shift_left_rem(size_t idx, array_t const& arr) const noexcept
    {
        return (idx == 0) ? base_t(0) :
                            ce_right_shift(base_t(arr[idx - 1] & m_shft_left_pattern), m_shft_leftright_shift);
    }

    template <size_t... S>
    constexpr array_t shift_right_impl(array_t const& arr, std::index_sequence<S...>) const noexcept
    {
        return {{h_shift_right(S, arr)...}};
    }

    constexpr base_t h_shift_right(size_t idx, array_t const& arr) const noexcept
    {
        return (idx + m_shft_div >= n_words) ?
                   base_t(0) :
                   base_t((arr[idx + m_shft_div] >> m_shft_mod) | h_shift_right_rem(idx + m_shft_div, arr));
    }

    constexpr base_t h_shift_right_rem(size_t idx, array_t const& arr) const noexcept
    {
        return (idx + 1 >= n_words) ?
                   base_t(0) :
                   ce_left_shift(base_t(arr[idx + 1] & m_shft_right_pattern), m_shft_leftright_shift);
    }

    template <size_t... S>
    constexpr array_t rotate_left_impl(array_t const& arr, this_t lft, this_t rgt,
                                       std::index_sequence<S...>) const noexcept
    {
        return {{(S > lft.m_shft_div ? lft.h_shift_left(S, arr) :
                                       S == lft.m_shft_div ? base_t(lft.h_shift_left(lft.m_shft_div, arr)
