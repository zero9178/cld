    ...
    const std::map<std::string, std::reference_wrapper<const Type>>& typedefs)
{
        ...
        auto result = std::visit(
            overload{
                ...
                [&typeSpecifiers, isConst, isVolatile,
                 &typedefs](const std::unique_ptr<Syntax::StructOrUnionSpecifier>& structOrUnion) -> ThisReturnType {
    ...
    auto result = std::visit(
        Y{overload{
            [&baseType, &getQualifiers, &typedefs](
                auto&& self, const Syntax::AbstractDeclarator* abstractDeclarator) -> std::optional<FailureReason> {
                    ...
                    Y{overload{
                        ...
                        [&baseType, &typedefs](auto&& directSelf,
                                               const Syntax::DirectAbstractDeclaratorParameterTypeList&
                                                   parameterTypeList) -> std::optional<FailureReason> {
                            ...
                            baseType =
                                FunctionType::create(std::move(baseType), std::move(arguments),
            ...
            [&baseType, &getQualifiers,
             &typedefs](auto&& self,
                        std::reference_wrapper<const Syntax::Declarator> declarator) -> std::optional<FailureReason> {
                    ...
                    Y{overload{
                        ...
                        [&](auto&& directSelf,
                            ...
                            auto [isConst, isVolatile, isRestricted] =
                                getQualifiers(dirWithoutStaticOrAsterisk.getTypeQualifiers());
                            if (!dirWithoutStaticOrAsterisk.getAssignmentExpression())
                            {
                                baseType =
                                    AbstractArrayType::create(isConst, isVolatile, isRestricted, std::move(baseType));
                            }
                            else
                            {
                                Constant::ConstantEvaluator evaluator;
                                ...
                                {
                                    baseType =
                                        ValArrayType::create(isConst, isVolatile, isRestricted, std::move(baseType));
                                }
                                else
                                {
                                    auto size = std::visit(
                        ...
                        [&](auto&& directSelf, const Syntax::DirectDeclaratorStatic& directDeclaratorStatic)
                            -> std::optional<FailureReason> {
                            auto [isConst, isVolatile, isRestricted] =
                                getQualifiers(directDeclaratorStatic.getTypeQualifiers());
                            Constant::ConstantEvaluator evaluator;
                            ...
                            else
                            {
                                ...
                                baseType =
                                    ArrayType::create(isConst, isVolatile, isRestricted, std::move(baseType), *size);
                            }
                            return std::visit(
                                [&directSelf](auto&& value) -> std::optional<FailureReason> {
                                    return directSelf(value);
                        ...
                        [&](auto&& directSelf, const Syntax::DirectDeclaratorAsterisk& directDeclaratorAsterisk)
                            -> std::optional<FailureReason> {
                            auto [isConst, isVolatile, isRestricted] =
                                getQualifiers(directDeclaratorAsterisk.getTypeQualifiers());
                            baseType = ValArrayType::create(isConst, isVolatile, isRestricted, std::move(baseType));
                            return std::visit(
                                [&directSelf](auto&& value) -> std::optional<FailureReason> {
                                    return directSelf(value);
                        ...
                        [&](auto&& directSelf, const Syntax::DirectDeclaratorParentheseParameters& parentheseParameters)
                            -> std::optional<FailureReason> {
                            std::vector<Type> arguments;
                            for (auto& pair : parentheseParameters.getParameterTypeList()
                                                  .getParameterList()
                                                  .getParameterDeclarations())
                            {
                                std::vector<SpecifierQualifierRef> specifierQualifiers;
                                    ...
                                    auto optional = std::visit(
                                        overload{[&specifierQualifiers](const Syntax::TypeSpecifier& typeSpecifier)
                                                 ...
                                                 [](const Syntax::StorageClassSpecifier&)
                                                     -> std::optional<FailureReason> { return {}; },
                                                 [](Syntax::FunctionSpecifier) -> std::optional<FailureReason> {
                                                     return FailureReason("Inline not allowed in paramter type list");
                                                 }},
                                        iter);
                                    if (optional)
                                    {
                                    ...
                                    std::visit(
                                        overload{
                                            ...
                                            [](const std::unique_ptr<Syntax::AbstractDeclarator>& abstractDeclarator)
                                                -> PossiblyAbstractQualifierRef { return abstractDeclarator.get(); }},
                                        pair.second),
                                    typedefs);
                                if (!result)
                                {
