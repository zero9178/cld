...
//    const auto endLine = sourceInterface.getLineNumber(TEST_ME_FILE_ID, actualEnd.getOffset() +
//    actualEnd.getLength()); const auto endOffset = sourceInterface.getLineEndOffset(TEST_ME_FILE_ID, endLine);
//
//    if (!m_after)
//    {
...
//                auto endToken = toMacroId0(*(annotate.maybeEnd().value_or(annotate.begin() + 1) - 1),
//                sourceInterface); auto begin = toMacroId0(*annotate.begin(), sourceInterface).getOffset();
//                CLD_ASSERT(begin >= startOffset);
...
//            [](auto&&) {});
//    }
//    // Sorting the underlines so that the character of underlines that start further to the right are the ones that
//    are
//    // printed
