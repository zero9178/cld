                ...
                for (auto& [specifiers, paramDeclarator] : parameterDeclarations)
                {
                    ...
                    if (auto* abstractDecl = std::get_if<std::unique_ptr<AbstractDeclarator>>(&paramDeclarator))
                    {
                        ...
                        context.log(Errors::Parser::MISSING_PARAMETER_NAME.args(
                            *pos, context.getSourceInterface(),
                                                                                  ...
                                                                                  bool pushScope)
{
    ...
    if (braceSeen)
    {
        additional = Notes::TO_MATCH_N_HERE.args(*start, context.getSourceInterface(), *start);
    }
    ...
    cld::Parser::parseHeadWhileStatement(Lexer::CTokenIterator& begin, Lexer::CTokenIterator end, Context& context)
{
    ...
    if (openPpos)
    {
        note = Notes::TO_MATCH_N_HERE.args(**openPpos, context.getSourceInterface(), **openPpos);
    }
    ...
    cld::Parser::parseFootWhileStatement(Lexer::CTokenIterator& begin, Lexer::CTokenIterator end, Context& context)
{
    ...
    if (doPos)
    {
        note = Notes::TO_MATCH_N_HERE.args(**doPos, context.getSourceInterface(), **doPos);
    }
